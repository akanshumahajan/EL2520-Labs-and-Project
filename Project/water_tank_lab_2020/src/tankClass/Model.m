    ...
    methods
        ...
        function WriteData(obj,data)
            ...
            if isnan(obj.History(end-delay,1))
                ...
                g = 981;   %cm/s^2
                noise_level = obj.model.noise_level;
%                 if obj.tap_flag
...
%%%% Need to update to 2 control signals
                interface_state(1,1) = interface_state(1,1) - obj.Ts*(alpha1*sqrt(2*g*interface_state(1,1)) - alpha13*sqrt(2*g*interface_state(1,3)) - beta1*obj.History_con(1,1)*(1+noise_level*randn));
                interface_state(1,2) = interface_state(1,2) - obj.Ts*(alpha2*sqrt(2*g*interface_state(1,2)) - alpha24*sqrt(2*g*interface_state(1,4)) - beta2*obj.History_con(1,2)*(1+noise_level*randn)); %control u2
                interface_state(1,3) = interface_state(1,3) - obj.Ts*(alpha3*sqrt(2*g*interface_state(1,3)) - beta32*obj.History_con(1,2)*(1+noise_level*randn)); %control u2
                interface_state(1,4) = interface_state(1,4) - obj.Ts*(alpha4*sqrt(2*g*interface_state(1,4)) - beta41*obj.History_con(1,1)*(1+noise_level*randn));
                % --- INTERFACE STATE --- %
